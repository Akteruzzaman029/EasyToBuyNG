...
export class CategoryTreeComponent implements OnInit {
  ...
  nzEvent(event: NzFormatEmitEvent): void {
    if (event.eventName === 'expand') {
      ...
      if (
        ...
        this.loadChildren(parentId).subscribe({
          next: (res: any) => {
            node.addChildren(this.mapToTreeNodes(res));
          },
          error: (err: any) => {
  ...
  loadChildren(parentId: number) {
    const payload: any = {
      name: '',
      companyId: CommonHelper.GetComapyId(),
      parentId,
    };

    return this.http.Post(`Category/GetCategoryTree`, payload);
  }
  constructor(
  ...
  mapToTreeNodes(data: any[]): NzTreeNodeOptions[] {
    return data.map((x) => ({
      title: x.name,
      key: x.id.toString(),
      isLeaf: !x.hasChild,
